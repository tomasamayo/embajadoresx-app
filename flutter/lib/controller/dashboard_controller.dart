...
class DashboardController extends GetxController {
  ...
  final RxBool isVendorMode =
      false.obs; // REQUERIMIENTO V1.2.2: Toggle de Modo Proveedor
  final RxString currentRank =
      "Nivel Inicial".obs; // REQUERIMIENTO V1.2.3: Rango observable
  final RxDouble excoinBalance =
      0.0.obs; // REQUERIMIENTO v2.0.7: Balance de ExCoin en Dashboard
  final RxString userId = ""
      .obs; // REQUERIMIENTO v5.3.0: ID de usuario observable para canje dinámico
  ...
  int get currentSuggestionIndex =>
      _currentSuggestionIndex >= 0 ? _currentSuggestionIndex : 0;
  int get totalSuggestionCount =>
      _cachedSuggestions.isEmpty ? 0 : _cachedSuggestions.length;
  ...
  Future<void> _loadSavedSuggestionIndex() async {
    _currentSuggestionIndex =
        preferences.getInt('current_suggestion_index') ?? -1;
  }

  // Save the current suggestion index to preferences
  Future<void> _saveSuggestionIndex() async {
    await preferences.setInt(
        'current_suggestion_index', _currentSuggestionIndex);
  }
  ...
  Future<void> toggleVendorMode() async {
    ...
    String endPoint =
        isVendorMode.value ? 'user/become_affiliate' : 'user/become_vendor';

    try {
      final response =
          await ApiService.instance.postData2(endPoint, {}, token: token);
  ...
  Future<void> fetchVerificationStatus() async {
    ...
    try {
      ...
      debugPrint('🧪 [VERIFICACIÓN RAW] $response');

      if (response != null && response['status'] == true) {
        final data = response['data'];
        isVerified.value =
            int.tryParse(data['is_verified']?.toString() ?? '0') ?? 0;

        final int? extractedId = data is Map<String, dynamic>
            ? SessionManager.extractUserId(data)
            : null;
        if (extractedId != null && extractedId > 0) {
          final String idStr = extractedId.toString();
          userId.value = idStr;
          await SessionManager.instance.setUserId(idStr);
          if (_loginModel?.data != null) {
            _loginModel!.data!.userId = idStr;
            await SharedPreference.setUserData(_loginModel!);
          }
          debugPrint(
              '✅ [VERIFICACIÓN] userId sincronizado desde api/get_user_details: $idStr');
        }
    ...
    try {
      ...
      if (response != null && response is Map<String, dynamic>) {
        ...
        if ((model.data?.userId == null ||
                model.data!.userId.toString().isEmpty ||
                model.data!.userId.toString() == 'null') &&
            extractedId != null &&
            extractedId > 0) {
          model.data?.userId = extractedId.toString();
        }
        updateUserData(model);
        await SharedPreference.setUserData(model);

        if (model.data?.planName != null &&
            model.data!.planName!.isNotEmpty &&
            model.data!.planName != "N/A") {
      ...
      if (value != null && value is Map<String, dynamic>) {
        ...
        if (data != null &&
          ...
          if (userPlan['is_active'].toString() == "1") {
            ...
            planName.value =
                userPlan['plan_name'].toString().split(' (')[0].trim();
          } else {
        ...
        if (data != null &&
            data is Map<String, dynamic> &&
            data.containsKey('excoin_balance')) {
          excoinBalance.value =
              double.tryParse(data['excoin_balance'].toString()) ?? 0.0;
          print(
              '✅ [DASHBOARD] ExCoin Balance actualizado: ${excoinBalance.value}');
        }
        ...
        if (data != null &&
            ...
            data['user_plan'] != null) {
          var userPlan = data['user_plan'];
          if (userPlan['current_rank_name'] != null &&
              userPlan['current_rank_name'].toString().isNotEmpty) {
        ...
        if (model.data.userPlan.isLifetime == "1") {
          ...
          final date2 = DateTime(
              model.data.userPlan.expireAt.year,
              model.data.userPlan.expireAt.month,
              model.data.userPlan.expireAt.day);
    ...
    if (_cachedSuggestions.length > 1) {
      ...
      do {
        newIndex = DateTime.now().millisecond % _cachedSuggestions.length;
          ...
          newIndex == _currentSuggestionIndex && _cachedSuggestions.length > 1);
  ...
  String getAISuggestion() {
    ...
    if (_cachedSuggestions.isEmpty) {
      ...
      if (_currentSuggestionIndex >= 0 &&
          _currentSuggestionIndex < _cachedSuggestions.length) {
        // Use the saved index
      } else {
        // Otherwise generate a new index
        _currentSuggestionIndex =
            DateTime.now().millisecond % _cachedSuggestions.length;
        _saveSuggestionIndex();
      }
    }
  ...
  List<String> _generateAdvancedAIInsights() {
    ...
    final balanceAmount =
        double.tryParse(userBalance.replaceAll(RegExp(r'[^\d.]'), "")) ?? 0;
    ...
    final weekAmount =
        double.tryParse(weekEarnings.replaceAll(RegExp(r'[^\d.]'), "")) ?? 0;
    final monthAmount =
        double.tryParse(monthEarnings.replaceAll(RegExp(r'[^\d.]'), "")) ?? 0;
    final yearAmount =
        double.tryParse(yearEarnings.replaceAll(RegExp(r'[^\d.]'), "")) ?? 0;

    // Wallet and commission data
    final walletUnpaidAmount =
        totals.walletUnpaidAmount is num ? totals.walletUnpaidAmount : 0.0;
    final walletUnpaidCount =
        totals.walletUnpaidCount is num ? totals.walletUnpaidCount : 0;

    // Click and sale data
    final totalClicksCount =
        totals.totalClicksCount is num ? totals.totalClicksCount : 0;
    final totalClicksCommission = totals.totalClicksCommission is num
        ? totals.totalClicksCommission
        : 0.0;
    final saleLocalstoreCount =
        totals.saleLocalstoreCount is num ? totals.saleLocalstoreCount : 0;
    final saleLocalstoreTotal =
        totals.saleLocalstoreTotal is num ? totals.saleLocalstoreTotal : 0.0;
    final saleLocalstoreCommission = totals.saleLocalstoreCommission is num
        ? totals.saleLocalstoreCommission
        : 0.0;

    // External and local store data
    final clickLocalstoreTotal = totals.clickLocalstoreTotal;
    final clickExternalTotal =
        totals.clickExternalTotal is num ? totals.clickExternalTotal : 0;
    final orderExternalCount = int.tryParse(totals.orderExternalCount) ?? 0;

    // Action and form data
    final clickActionTotal =
        totals.clickActionTotal is num ? totals.clickActionTotal : 0;
    final clickFormTotal =
        totals.clickFormTotal is num ? totals.clickFormTotal : 0;

    // Vendor data
    final vendorClickLocalstoreTotal = totals.vendorClickLocalstoreTotal is num
        ? totals.vendorClickLocalstoreTotal
        : 0;
    final vendorSaleLocalstoreCount = totals.vendorSaleLocalstoreCount is num
        ? totals.vendorSaleLocalstoreCount
        : 0;

    // Refer data
    final referStatus = data.referStatus;
    final uniqueResellerLink = data.uniqueResellerLink;
    final totalProductClicks =
        int.tryParse(referTotal.totalProductClick.clicks) ?? 0;
    final totalGeneralClicks =
        int.tryParse(referTotal.totalGaneralClick.totalClicks) ?? 0;
    final totalActionClicks =
        int.tryParse(referTotal.totalAction.clickCount) ?? 0;
    final totalProductSalesCounts =
        int.tryParse(referTotal.totalProductSale.counts) ?? 0;
    ...
    final date1 = DateTime(now.year, now.month, now.day);
    final date2 = DateTime(
        userPlan.expireAt.year, userPlan.expireAt.month, userPlan.expireAt.day);
    ...
    if (walletUnpaidAmount > 0 && walletUnpaidCount > 0) {
      if (balanceAmount > 0) {
        ...
        insights.add(
            "He analizado tu cuenta: tienes $currency$balanceAmount disponibles más $currency$walletUnpaidAmount en comisiones pendientes de $walletUnpaidCount transacciones sin pagar. Tu potencial total es $currency$totalAvailable. Considera retirar algunos fondos mientras continúas aumentando tus ganancias.");
      } else {
        insights.add(
            "Tienes $walletUnpaidCount comisiones sin pagar esperándote, por un total de $currency$walletUnpaidAmount. Solicitar un retiro convertiría estas comisiones pendientes en fondos disponibles.");
      }
    }

    // 2. PERFORMANCE TREND INSIGHT
    if (weekAmount > 0 && monthAmount > 0 && yearAmount > 0) {
      final weekPercentageOfMonth =
          (weekAmount / monthAmount * 100).toStringAsFixed(1);
      final monthPercentageOfYear =
          (monthAmount / yearAmount * 100).toStringAsFixed(1);

      if (double.parse(weekPercentageOfMonth) > 25) {
        insights.add(
            "¡Estás superando tu promedio! Las ganancias de esta semana ($currency$weekAmount) representan el $weekPercentageOfMonth% de tu total mensual ($currency$monthAmount), lo cual está por encima de la tasa semanal esperada del 25%. Mantén este impulso para aumentar significativamente tus ganancias mensuales.");
      } else {
        insights.add(
            "Las ganancias de esta semana ($currency$weekAmount) representan el $weekPercentageOfMonth% de tus ganancias mensuales ($currency$monthAmount), lo cual está ${double.parse(weekPercentageOfMonth) < 25 ? "por debajo" : "en"} el ritmo semanal esperado. Tu mes está actualmente en el $monthPercentageOfYear% de tus ganancias anuales.");
      }
    }
    ...
    if (marketTools.isNotEmpty) {
      ...
      final bestClickTool =
          marketTools.reduce((a, b) => a.clickCount > b.clickCount ? a : b);
      // Find best performing tool by sale count
      final bestSaleTool =
          marketTools.reduce((a, b) => a.saleCount > b.saleCount ? a : b);

      if (bestClickTool.clickCount > 0) {
        insights.add(
            "Tu herramienta con mejor rendimiento por tráfico es \"${bestClickTool.title}\" con ${bestClickTool.clickCount} clics. ${bestClickTool.saleCount > 0 ? "Ha generado ${bestClickTool.saleCount} ventas hasta ahora." : "Considera optimizar tu embudo para convertir más de este tráfico en ventas."}");
      }

      if (bestSaleTool.saleCount > 0 &&
          bestSaleTool.title != bestClickTool.title) {
        insights.add(
            "\"${bestSaleTool.title}\" es tu mejor conversor con ${bestSaleTool.saleCount} ventas. Su tasa de conversión del ${(bestSaleTool.saleCount / (bestSaleTool.clickCount > 0 ? bestSaleTool.clickCount : 1) * 100).toStringAsFixed(1)}% es impresionante. Considera dirigir más tráfico a esta herramienta de alto rendimiento.");
      }

      // Find tools with zero clicks but good commission potential
      final unusedTools = marketTools
          .where((tool) =>
              tool.clickCount == 0 &&
              (double.tryParse(tool.saleCommisionYouWillGet
      ...
      if (unusedTools.isNotEmpty && unusedTools.length <= 3) {
        ...
        insights.add(
            "He identificado un potencial sin explotar: $toolNames ${unusedTools.length == 1 ? "no está" : "no están"} generando tráfico aún, pero ofrece${unusedTools.length == 1 ? "" : "n"} buenas tasas de comisión. Intenta promocionar ${unusedTools.length == 1 ? "esta herramienta" : "estas herramientas"} para diversificar tus fuentes de ingresos.");
      }
    }
    ...
    if (totalClicksCount > 0) {
      if (saleLocalstoreCount > 0) {
        final conversionRate =
            (saleLocalstoreCount / totalClicksCount * 100).toStringAsFixed(2);
        insights.add(
        ...
        insights.add(
            "Has generado $totalClicksCount clics pero aún no has convertido ninguna venta. Con este volumen de tráfico, enfócate en optimizar tu embudo: verifica que las páginas de destino sean atractivas y que los productos coincidan con los intereses de tu audiencia.");
      } else {
        insights.add(
            "Has comenzado a generar clics ($totalClicksCount hasta ahora) pero necesitas más tráfico antes de enfocarte en la optimización de la conversión. Continúa construyendo tu audiencia para alcanzar al menos 100 clics para obtener datos de conversión significativos.");
      }
    }

    // 5. COMMISSION EFFICIENCY ANALYSIS
    if (totalClicksCount > 0 && totalClicksCommission > 0) {
      final earningsPerClick =
          (totalClicksCommission / totalClicksCount).toStringAsFixed(2);
      insights.add(
          "Tu comisión promedio por clic es $currency$earningsPerClick. ${double.parse(earningsPerClick) > 0.15 ? "Esta es una relación de ganancias sólida, lo que indica que estás promocionando productos de alto valor o tienes términos de comisión favorables." : "Considera promocionar productos con tasas de comisión más altas para maximizar tu retorno sobre el tráfico."}");
    }

    // 6. TRAFFIC SOURCE ANALYSIS
    if (clickLocalstoreTotal > 0 ||
        clickExternalTotal > 0 ||
        clickActionTotal > 0 ||
        clickFormTotal > 0) {
      final totalTraffic = clickLocalstoreTotal +
          (clickExternalTotal as num) +
          (clickActionTotal as num) +
          (clickFormTotal as num);

      if (totalTraffic > 0) {
        final localPercent =
            (clickLocalstoreTotal / totalTraffic * 100).toStringAsFixed(1);
        final externalPercent =
            (clickExternalTotal / totalTraffic * 100).toStringAsFixed(1);
        final actionPercent =
            (clickActionTotal / totalTraffic * 100).toStringAsFixed(1);
        final formPercent =
            (clickFormTotal / totalTraffic * 100).toStringAsFixed(1);
    ...
    if (!referStatus && totalProductClicks == 0 && totalGeneralClicks == 0) {
      insights.add(
          "Tu programa de referidos está actualmente deshabilitado. Habilitar esta función podría crear un flujo de ingresos pasivos a través de tu red. Considera activarlo en la configuración de tu cuenta para expandir tu potencial de ganancias.");
    } else if (referStatus &&
        (totalProductClicks > 0 || totalGeneralClicks > 0)) {
      if (totalProductSalesCounts > 0) {
        insights.add(
            "Tu programa de referidos está generando resultados con $totalGeneralClicks clics generales, $totalProductClicks clics de productos y $totalProductSalesCounts ventas. Continúa compartiendo tu enlace único de revendedor para aumentar estos números.");
      } else {
        insights.add(
            "Tu programa de referidos ha generado tráfico ($totalGeneralClicks clics generales y $totalProductClicks clics de productos) pero aún no se ha convertido en ventas. Considera entrenar a tus referidos sobre estrategias de promoción efectivas para mejorar las conversiones.");
      }
    } else if (referStatus &&
        uniqueResellerLink.isNotEmpty &&
        totalProductClicks == 0 &&
        totalGeneralClicks == 0) {
      insights.add(
          "Tu programa de referidos está activo pero aún no ha generado tráfico. Comparte tu enlace único de revendedor con socios potenciales o en tus redes sociales para comenzar a construir tu red de referidos.");
    }
    ...
    if (isMembershipAccess && !isLifetime && daysLeft > 0) {
      if (daysLeft <= 7) {
        insights.add(
            "⚠️ ¡Tu membresía expira en solo $daysLeft día${daysLeft == 1 ? '' : 's'}! Para mantener el acceso a todas las herramientas y funciones de afiliados, renueva tu suscripción pronto para evitar cualquier interrupción en tus ganancias.");
      } else if (daysLeft <= 30) {
        insights.add(
            "Tu membresía expirará en $daysLeft días. Si estás viendo resultados positivos, considera renovar para mantener tu impulso. Tu plan actual dura hasta el ${Jiffy.parseFromDateTime(userPlan.expireAt).yMMMMd}.");
      }
    } else if (!isMembershipAccess) {
      insights.add(
          "Actualmente no tienes acceso activo a la membresía. Actualizar a un plan pago desbloquearía herramientas de afiliados adicionales, tasas de comisión más altas y más oportunidades de promoción.");
    }

    // 9. STORE INSIGHT
    if (storeUrl.isEmpty) {
      insights.add(
          "Noto que la URL de tu tienda de afiliados no está configurada. Configurar esto crearía una tienda dedicada para tus productos, aumentando potencialmente tus tasas de conversión y brindando una experiencia más profesional a tus clientes.");
    } else if (storeUrl.isNotEmpty &&
        saleLocalstoreCount == 0 &&
        clickLocalstoreTotal > 20) {
      insights.add(
          "Tu tienda de afiliados está recibiendo tráfico ($clickLocalstoreTotal clics) pero aún no ha generado ventas. Considera revisar el diseño de tu tienda, la selección de productos y los elementos de llamada a la acción para mejorar la experiencia del cliente.");
    } else if (storeUrl.isNotEmpty && saleLocalstoreCount > 0) {
      final storeConversionRate =
          (saleLocalstoreCount / clickLocalstoreTotal * 100).toStringAsFixed(2);
      insights.add(
          "Tu tienda de afiliados tiene una tasa de conversión del $storeConversionRate% ($saleLocalstoreCount ventas de $clickLocalstoreTotal clics). ${double.parse(storeConversionRate) > 2.5 ? "Esto está por encima del promedio para el comercio electrónico, lo que indica un diseño de tienda efectivo." : "La tasa de conversión promedio del comercio electrónico es del 2-3%, por lo que puede haber espacio para optimizar el diseño de tu tienda o la presentación del producto."}");
    }

    // 10. VENDOR PERFORMANCE INSIGHT
    if (vendorClickLocalstoreTotal > 0 || vendorSaleLocalstoreCount > 0) {
      if (vendorSaleLocalstoreCount > 0) {
        final vendorConversionRate = (vendorSaleLocalstoreCount /
                (vendorClickLocalstoreTotal > 0
                    ? vendorClickLocalstoreTotal
                    : 1) *
                100)
            .toStringAsFixed(2);
        insights.add(
            "Como vendedor, tus productos han logrado una tasa de conversión del $vendorConversionRate% ($vendorSaleLocalstoreCount ventas de $vendorClickLocalstoreTotal clics). ${double.parse(vendorConversionRate) > 3 ? "Este es un rendimiento sólido, lo que sugiere que tus productos resuenan bien con los compradores." : "Considera optimizar tus listados de productos o estrategia de precios para mejorar las conversiones."}");
      } else if (vendorClickLocalstoreTotal > 0) {
        insights.add(
            "Tus productos de vendedor están recibiendo atención ($vendorClickLocalstoreTotal clics) pero aún no se han convertido en ventas. Revisa las descripciones, precios e imágenes de tus productos para hacer que tus ofertas sean más atractivas para los compradores potenciales.");
      }
    }
    ...
    if (orderExternalCount > 0) {
      insights.add(
          "Has generado $orderExternalCount pedidos externos. Esto muestra tu capacidad para impulsar conversiones a través de plataformas externas. Continúa aprovechando estos canales mientras expandes tu alcance promocional para maximizar las ventas multiplataforma.");
    }
    ...
    if (insights.length < 3) {
      if (totalClicksCount > 0 ||
          ...
          balanceAmount > 0) {
        insights.add(
            "Estoy monitoreando activamente tu rendimiento de marketing de afiliados. A medida que generes más datos a través de promociones continuas, proporcionaré información cada vez más detallada y procesable para ayudar a optimizar tu estrategia.");
      } else {
        insights.add(
            "Tu cuenta está configurada y lista para actividades de marketing de afiliados. Comienza promocionando tus enlaces a través de marketing de contenido, redes sociales o campañas de correo electrónico para generar tus primeros clics y datos de ventas.");
      }
    }
